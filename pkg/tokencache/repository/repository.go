...
func computeChecksum(key tokencache.Key) (string, error) {
	s := sha256.New()
	var err error
	// Use JSON when either map has multiple entries because it encodes map keys deterministically.
	// Use gob otherwise for compatibility with cache keys generated by earlier versions.
	if len(key.AuthRequestExtraParams) > 1 || len(key.Provider.RequestHeaders) > 1 {
		err = json.NewEncoder(s).Encode(&key)
	} else {
		err = gob.NewEncoder(s).Encode(&key)
	}
	if err != nil {
